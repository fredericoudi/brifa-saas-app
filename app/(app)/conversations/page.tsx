...
export default async function ConversationsPage() {
  ...
  let overview: Awaited<ReturnType<typeof getAgencyConversationOverview>> | null = null;
  let loadError: string | null = null;

  try {
    overview = await getAgencyConversationOverview(profile.agency_id);
  } catch (error) {
    loadError = error instanceof Error ? error.message : "Falha ao carregar a camada conversacional da agência.";
  }
    ...
    <div className="space-y-6">
      <section className="grid gap-4 md:grid-cols-4">
        <MetricCard
          ...
          subtitle={activeChannel ? getConversationProviderLabel(activeChannel.provider) : "Nenhum canal configurado"}
        />
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-base font-semibold">Integração conversacional</h2>
              <p className="mt-1 text-sm text-muted">
                Configure o canal do WhatsApp da agência e acompanhe a operação conversacional do BRIFA.
              </p>
            </div>
            {activeChannel ? (
        ...
        <CardContent>
          <ConversationChannelManager initialChannel={activeChannel} webhookUrl={webhookUrl} />
        </CardContent>
      </Card>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <p className="mt-1 text-sm text-muted">Histórico recente das mensagens processadas por esta agência.</p>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <p className="mt-1 text-sm text-muted">Cada execução fica registrada aqui para auditoria e depuração.</p>
          </CardHeader>
          <CardContent>
